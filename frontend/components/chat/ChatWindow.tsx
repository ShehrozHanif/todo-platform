...
// OpenAI ChatKit integration — renders the ChatKit web component when domain
// verification succeeds, otherwise uses a ChatKit-styled fallback that talks
// to the same backend (OpenAI Agents SDK + MCP via chatkit-python).
'use client';
...
// ---------------------------------------------------------------------------
// Attempt to mount the OpenAI ChatKit web component (loaded from CDN).
// If domain verification fails the element renders blank — we detect that
// and hide it so the fallback chat is visible underneath.
// ---------------------------------------------------------------------------
function ChatKitEmbed() {
  ...
  useEffect(() => {
    ...
    async function tryMount() {
      ...
      // Wait for the CDN script to define the custom element (max 6s)
      if (!customElements.get('openai-chatkit')) {
        try {
          ...
          return; // CDN didn't load — keep hidden
        }
      }
      ...
      try {
        ...
        return; // setOptions failed — keep hidden
      }

      // Wait a moment, then check if the element actually rendered
      // (domain verification happens async inside the web component)
      await new Promise(r => setTimeout(r, 3000));
      if (cancelled) return;

      const rect = el.getBoundingClientRect();
      const shadow = el.shadowRoot;
      // If the element has content (height > 50px or shadow DOM exists with content)
      if (rect.height > 50 || (shadow && shadow.childElementCount > 0)) {
        setVisible(true);
      } else {
...
// ---------------------------------------------------------------------------
// Main chat UI — always visible, works with the existing chat endpoint
// styled to match ChatKit aesthetic. Backend uses openai-chatkit (Python SDK).
// ---------------------------------------------------------------------------
...
export function ChatWindow() {
  ...
  return (
    <div className="relative h-full">
      {/* ChatKit web component overlay — mounts on top if domain verification passes */}
      <ChatKitEmbed />

      {/* Primary chat UI — always renders, hidden only if ChatKit takes over */}
      <div className="flex flex-col h-full">
        {/* Messages area */}
        <div className="flex-1 overflow-y-auto px-4 md:px-6 py-4 space-y-4">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-4 py-8">
              {/* OpenAI ChatKit branding */}
